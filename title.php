<?php
global /** @var \framework\TDFramework $TD */
$TD, $wp_query;
...
if (!bridge_qode_is_title_hidden()) { ?>
    ...
    if ($bridge_qode_responsive_title_image == 'yes' && $bridge_qode_show_title_image == true) {
        echo ' with_image';
    } ?>" <?php print bridge_qode_get_module_part($bridge_qode_animation); ?>  <?php echo 'data-height="' . $bridge_qode_title_height . '"';
    if ($bridge_qode_title_height != '' && $bridge_qode_animate_title_area == 'area_top_bottom') {
        echo 'style="opacity:0;height:' . $bridge_qode_header_height_padding . 'px;"';
    } ?>>
        <div class="title <?php echo bridge_qode_get_module_part($bridge_qode_page_title_fontsize . " " . $bridge_qode_page_title_position . " " . $bridge_qode_page_title_border_bottom_in_grid_class);
        if ($bridge_qode_responsive_title_image == 'no' && $bridge_qode_title_image != "" && ($bridge_qode_fixed_title_image == "yes" || $bridge_qode_fixed_title_image == "yes_zoom") && $bridge_qode_show_title_image == true) {
            echo ' has_fixed_background ';
            if ($bridge_qode_fixed_title_image == "yes_zoom") {
                echo 'zoom_out ';
            }
        }
        if ($bridge_qode_responsive_title_image == 'no' && $bridge_qode_title_image != "" && $bridge_qode_fixed_title_image == "no" && $bridge_qode_show_title_image == true) {
            echo ' has_background';
        } ?>"
             style="<?php if ($bridge_qode_responsive_title_image == 'no' && $bridge_qode_title_image != "" && $bridge_qode_show_title_image == true) {
                 if ($bridge_qode_title_image_width != '') {
                     echo 'background-size:' . $bridge_qode_title_image_width . 'px auto;';
                 }
                 echo 'background-image:url(' . $bridge_qode_title_image . ');';
             }
             if ($bridge_qode_title_height != '') {
                 echo 'height:' . $bridge_qode_title_height . 'px;';
             }
             if ($bridge_qode_title_background_color != '') {
                 echo 'background-color:' . $bridge_qode_title_background_color . ';';
             } ?>">
            <div class="image <?php if ($bridge_qode_responsive_title_image == 'yes' && $bridge_qode_title_image != "" && $bridge_qode_show_title_image == true) {
                echo "responsive";
            } else {
                echo "not_responsive";
            } ?>"><?php if ($bridge_qode_title_image != "" && $bridge_qode_show_title_image == true) { ?><img
                    itemprop="image" src="<?php echo esc_url($bridge_qode_title_image); ?>" alt="&nbsp;" /> <?php } ?>
            </div>
            <?php if ($bridge_qode_title_overlay_image != "") { ?>
                <div class="title_overlay"
                     style="background-image:url('<?php echo esc_url($bridge_qode_title_overlay_image); ?>');"></div>
            <?php } ?>
            <?php if (!bridge_qode_is_title_text_hidden()) { ?>
                <div class="title_holder" <?php print bridge_qode_get_module_part($bridge_qode_page_title_content_animation_data); ?> <?php if ($bridge_qode_responsive_title_image != 'yes' && get_post_meta($id, "qode_show-page-title-image", true) == "") {
                    echo bridge_qode_get_module_part($bridge_qode_title_holder_height);
                } ?>>
                    <div class="container">
                        <div class="container_inner clearfix">
                            <div class="title_subtitle_holder" <?php if ($bridge_qode_responsive_title_image == 'yes' && $bridge_qode_show_title_image == true) {
                                echo bridge_qode_get_module_part($bridge_qode_title_subtitle_padding);
                            } ?>>
                                <?php if (isset($bridge_qode_options['overlapping_content']) && $bridge_qode_options['overlapping_content'] == 'yes') { ?>
                                <div class="overlapping_content_margin"><?php } ?>
                                    ...
                                    <div class="title_subtitle_holder_inner">
                                        ...
                                        <?php } ?>
                                        <h1 <?php print bridge_qode_get_module_part($bridge_qode_page_title_animation_data);
                                        if (get_post_meta($id, "qode_page-title-color", true)) { ?> style="color:<?php echo get_post_meta($id, "qode_page-title-color", true) ?>" <?php } ?>>
                                            <span><?php bridge_qode_title_text(); ?></span></h1>
            ...
            <?php if ($bridge_qode_is_title_angled_enabled) { ?>
                <svg class="angled-section svg-title-bottom" preserveAspectRatio="none" viewBox="0 0 86 86" width="100%"
                     height="86">
                    <?php if ($bridge_qode_title_angled_position == 'from_left_to_right') { ?>
                        <polygon style="fill: <?php echo esc_attr($bridge_qode_title_angled_background_color); ?>;"
                                 points="0,0 0,86 86,86"/>
                    <?php }
                    if ($bridge_qode_title_angled_position == 'from_right_to_left') { ?>
                        <polygon style="fill: <?php echo esc_attr($bridge_qode_title_angled_background_color); ?>;"
                                 points="0,86 86,0 86,86"/>
                    <?php } ?>
                </svg>
            <?php } ?>
        </div>
